...
const { createZimaSimulator } = require('../electron/zima-simulator.cjs');
...
const main = async () => {
  ...
  const simulator = createZimaSimulator();
  if (replayPath) {
  ...
  const beaconIds = parseBeaconIds(args['beacon-ids']);
  const beaconCount = Number(args.beacons);
  const started = await simulator.start({
    ...
    commandPort: args['command-port'],
    bindInterface: args['bind-interface'],
    rateHz: args.rate,
    commandEcho: toBoolean(args['command-echo']) || toBoolean(args['wait-for-ocon']),
    waitForOcon: toBoolean(args['wait-for-ocon']),
    onlyValid: toBoolean(args['only-valid']),
    onlyBroken: toBoolean(args['only-broken']),
    beaconIds,
    ...(Number.isInteger(beaconCount) && beaconCount > 0 ? { beacons: beaconCount } : {}),
  });

  console.log('[zima-sim] started', started);
  ...
  const printStatus = async () => {
    ...
    console.log('[zima-sim] status', status);
  };
  ...
  const shutdown = async (signal) => {
    ...
    try {
      ...
      console.log(`[zima-sim] stopped by ${signal}`);
      process.exit(0);
    }
  };
...
void main().catch((error) => {
  console.error('[zima-sim] failed', error instanceof Error ? error.message : String(error));
  process.exit(1);
});
