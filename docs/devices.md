...
- Режим `divers`:
  ...
  - сопоставление `tID` выполняется по `mission.ui.divers[].beacon_id`.
- В обоих режимах сообщения `$PRWLA` обновляют позиции буёв RWLT (`buoyId` 1..4), буи отображаются отдельными маркерами.
