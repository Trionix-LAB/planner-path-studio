...
Формат сообщения: `@AZMLOC,stPressure_mBar,stDepth_m,waterTemp_C,stPitch_deg,stRoll_deg,age,lat_deg,lon_deg,course_deg,speed_mps,age,heading_deg,age,x_m,y_m,z_m,rerr_m,age`. Получается из выходного порта
...
| 14 | x_m | f32 | м | 0 .. | Вычисленное положение: координата X (Режим LBL) | Расчетное значение |
| 15 | y_m | f32 | м | 0 .. | Вычисленное положение: координата Y (Режим LBL) | Расчетное значение |
| 16 | z_m | f32 | м | 0 .. 300 | Глубина погружения устройства (значение аналогично значению поля №2) | Встроенный сенсор |
| 17 | rerr_m | f32 | м | 0 .. | Радиальная ошибка определения положения (Режим LBL) | Расчетное значение |
...
Формат сообщения: `@AZMREM,rem_addr,SRange_m,Azimuth_deg,PTime_s,MSR_dB,age,Depth_m,age,SRangeProjection_m,age,ADistance_m,age,AAzimuth_deg,age,Elevation_deg,age,VCC_V,age,WaterTemp_C,age,Lat_deg,Lon_deg,age,RAzimuth_deg,age,Message,age,X_m,Y_m,Z_m,IsTimeout`. Получается из выходного порта
...
| 30 | Z_m | f32 | м | - | Положение в локальной декартовой системе координат: координата Z (Режим LBL) | Заданное пользователем |
