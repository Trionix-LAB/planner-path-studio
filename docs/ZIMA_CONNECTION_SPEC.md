...
3.  **Фильтрация сообщений**:
    ...
    *   Перед анализом нормализовать строку: убрать управляющие символы (`NUL` и т.п.) и, если есть служебный префикс логирования, выделить полезную часть от `@AZMLOC`/`@AZMREM`.
    *   При получении `@AZMLOC`/`@AZMREM` извлекать соответствующие поля позиции и телеметрии по описанным в `docs/devices.md` схемам.

4.  **Извлечение полей**:
    *   Для `@AZMLOC` извлекать поля: lat_deg, lon_deg, heading_deg и другие параметры станции (pressure, depth, pitch, roll, speed и т. п.); учитывать, что строка может заканчиваться завершающей запятой.
    *   Для `@AZMREM` извлекать параметры маяков: rem_addr, SRange_m, Azimuth_deg, Depth_m и т. п. (см. `docs/devices.md`), поддерживая актуальный хвост `Message,age,IsTimeout` и legacy-варианты с `X_m,Y_m`/`X_m,Y_m,Z_m`.
...
## 7. Специфика сообщений `@AZMLOC` и `@AZMREM` (кратко)
 - `@AZMLOC` — локальные параметры станции/антенны. Обычно отправляется ~1 раз в секунду, содержит: давление, глубину, температуру, крен/дифферент, возраст данных, lat/lon/course/speed/heading. В актуальной версии протокола строка обычно завершается запятой.
 - `@AZMREM` — данные маяка-ответчика. Отправляется отдельно для каждого маяка при обновлении или таймауте. Содержит: адрес маяка, наклонную дальность (SRange), азимут прихода, PTime, MSR, глубину маяка, проекции, абсолютные азимуты, VCC, температуру, lat/lon, сообщение, возраст сообщения и признак таймаута. В legacy-версии могут присутствовать дополнительные `X_m/Y_m/(Z_m)`.
...
## 8. Контракт курса и скорости (runtime)

- Для базовой станции (`@AZMLOC`) курс и скорость берутся из сообщения `AZMLOC` (`course_deg`, `speed_mps`) без пересчета over-ground.
- Для агента (`@AZMREM`) курс/скорость вычисляются как `COG/SOG` по последовательным координатам `AZMREM` для одного и того же маяка (`rem_addr`).
- В UI `beacon_id` водолаза задается в диапазоне `1..16`.
- Для входящего `AZMREM.rem_addr` поддерживается legacy-нормализация `0..15 -> 1..16` по правилу `beacon_id = rem_addr + 1`.
- Нормализация `AZMREM.rem_addr` выполняется до сопоставления с агентом и не зависит от эвристик авто-смещения.
- Курс/скорость `AZMLOC` не должны использоваться как fallback для `AZMREM`.
- Если для назначенного `beacon_id` нет `AZMREM`, агент считается без телеметрии (`no fix`) до появления соответствующего сообщения.
- При недостатке данных для расчета over-ground (первая точка или невалидный `dt`) runtime возвращает `speed=0`, `course=0`.
- Контракт используется единообразно для HUD, ориентации маркера и записи трека.
