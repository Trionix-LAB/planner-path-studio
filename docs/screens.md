...
- Верхняя панель (toolbar):
  ...
  - В меню импорта должен быть явный путь для CAD-слоев:
    - `ToolBar -> Импортировать -> Импортировать DWG/DXF -> DWG/DXF + UTM -> Окно выбора зоны UTM`
    - окно UTM: номер зоны (`1..60`) и полушарие (`N/S`) по той же модели, что для `TIF + TFW (UTM зона)`.
    - должны поддерживаться оба входных формата: `.dxf` и `.dwg` (обработка каждого формата выполняется нативно, без конвертации между ними).
    - `DXF` разбирается нативным встроенным парсером `src/features/map/dxfOverlay/parseDxf.ts`.
    - `DWG` разбирается нативно через `@mlightcad/libredwg-web` (`src/features/map/dwgOverlay/parseDwg.ts`).
    - при импорте CAD на карту попадает только геометрия рисунка из model space; рамка листа/штамп/элементы layout (paper space) не импортируются и не участвуют в вычислении границ/центра слоя.
    - файлы наложений в папке миссии MUST храниться раздельно:
      - `overlays/rasters/` для TIF/TFW;
      - `overlays/vectors/` для DXF/DWG и `vector-cache`.
    - для ускорения повторного открытия миссии после первого импорта/парсинга сохраняется персистентный cache-файл геометрии (`overlays/vectors/*.vector-cache.json`), который переиспользуется при загрузке миссии.
    - отображение overlay-слоев выполняется в оптимизированном выделенном слое карты: кэшированные данные переиспользуются без повторного парсинга, а обновления HUD/телеметрии не должны триггерить полную перерисовку всех CAD-геометрий.
    - для тяжелых CAD-слоев карта использует canvas-preferred рендер Leaflet для улучшения отзывчивости pan/zoom.
    - для импортированных векторных слоев пользователь может изменить цвет отображения по ПКМ на строке слоя в секции `Векторные слои` (левая панель); новый цвет применяется сразу ко всем сущностям слоя (линии, точки, фигуры).
  - Блок "Инструменты": "Маршрут", "Зона (галсы)", "Маркер", "Измерить", "Выбор/Редактирование".
...
- Правая панель (dock):
  - HUD (координаты, скорость, курс, глубина) — отображает данные выбранного агента (при выборе агента в левой панели HUD переключается на его телеметрию).
    - при выборе базовой станции в левой панели HUD переключается на телеметрию базовой станции.
    - если не выбран ни агент/базовая станция и не выбран `rwlt_buoy`, HUD показывает `нет данных` и не отображает телеметрию последней выбранной сущности.
    - Для GNSS-источников курс берется из `HDT` при свежести не более `5` секунд, иначе вычисляется как `COG`; скорость вычисляется как `SOG`.
    - Для `Zima2R`: у базовой станции курс/скорость берутся из `AZMLOC`, у агента — вычисляются `COG/SOG` по `AZMREM`.
  ...
  - (опционально) «Объекты» (краткий список: маршруты/зоны/маркеры/RWLT-буи; быстрый выбор).
  - Свойства объекта.
  - Секции `HUD`, `Статус`, `Свойства объекта` поддерживают сворачивание/разворачивание по стрелке справа в заголовке секции (вправо = свернуто, вниз = развернуто).
  - Состояние сворачивания секций правой панели сохраняется в состоянии миссии/черновика и восстанавливается при повторном открытии.
  - Ширина правой панели изменяется перетаскиванием вертикального разделителя; текущее состояние ширины и свёрнутости панели сохраняется в миссии/черновике и восстанавливается при повторном открытии.
- Левая панель (dock):
  - Менеджер слоев (track/routes/markers/grid/scale_bar; слой водолаза не отключаемый).
  - Секции `Слои`, `Растры`, `Векторные слои`, `Агенты`, `Объекты` поддерживают сворачивание/разворачивание по стрелке справа в заголовке секции (вправо = свернуто, вниз = развернуто).
  - Состояние сворачивания секций левой панели сохраняется в состоянии миссии/черновика и восстанавливается при повторном открытии.
  - Ширина левой панели изменяется перетаскиванием вертикального разделителя; текущее состояние ширины и свёрнутости панели сохраняется в миссии/черновике и восстанавливается при повторном открытии.
  - Панель «Агенты»: список агентов миссии. Для каждого агента отображается:
    ...
    - при клике на «Базовая станция» HUD переключается на данные базовой станции
  - (опционально) «Объекты» (краткий список: маршруты/зоны/маркеры/RWLT-буи; быстрый выбор).
...
1) Реальное время:
   - метка агента — навигационная стрелка-шеврон (весь маркер ориентирован по курсу, аналог Google Maps); цвет маркера задаётся настройкой агента.
   - метка базовой станции — круглая иконка антенны с внешним SVG-указателем курса, который не перекрывает иконку; цвет маркера фиксированный, в настройках меняется только размер.
   - направление обоих маркеров строится по `course` согласно контракту `R-066` (см. [spec/spec.md](../spec/spec.md), `R-067`).
...
3) Инструменты планирования:
   ...
   - "Измерить": первый клик фиксирует начало, второй клик завершает измерение; до второго клика показывается превью линии до курсора; после завершения отображается пунктирная линия и подпись расстояния в метрах.
   - ПКМ по кнопке "Измерить": подтверждение удаления всех объектов измерений.
...
- `Esc`: отмена текущего действия/выход из режима рисования (включая незавершенное измерение).
...
- Формат:
  - Треки: GPX / KML / CSV
  - Маршруты: GPX / KML / CSV
  - Точки: CSV / GPX
  - Для каждого блока (`Треки` / `Маршруты и галсы` / `Точки-маркеры`) при выборе `CSV` доступны:
    - выбор CRS (`WGS84` / `СК-42` / `ГСК-2011`)
    - выбор формата координат (`ДД` / `ГМ` / `ГМС`)
...
- Блок "Измерения":
  ...
  - сетка рассчитывается в локальной UTM-зоне центра карты и рисуется в WGS84; для текущего экрана используется одна UTM-зона
  - при широтах вне диапазона UTM (`lat < -80` или `lat > 84`) слой сетки автоматически скрывается
...
- Блок "Координаты":
  ...
  - формат ввода координат по умолчанию для редакторов:
    - `Десятичные градусы`
...
- Блок "Оборудование" (отдельная вкладка):
  ...
  - список лог-файлов `logs/equipment/*.log` для инстансов активного профиля с кнопками скачивания
  - для длинного списка логов должен использоваться прокручиваемый контейнер (scrollable list)
...
1) Маршрут (`route`):
   ...
   - ввод координат через окно "Открыть координаты" с выбором CRS (`WGS84`/`СК-42`/`ГСК-2011`), выбором формата (`ДД`/`ГМ`/`ГМС`), масками полей и preview конвертации в WGS84
   - в этом окне кнопка `Ок` только закрывает окно; применение изменений выполняется кнопкой `Сохранить изменения` в правой панели

2) Зона (`survey_area`):
   - имя
   - заметка
   - цвет зоны
   - цвет галсов (отдельный контрол рядом с цветом зоны; влияет только на линии галсов текущей зоны)
   - lane_angle_deg (`0..360`, шаг `1`, абсолютный глобальный угол галсов)
   - lane_width_m (м)
   - ориентация галсов (read-only: `авто` или `по грани (N°)`)
   - старт галсов (read-only: координаты стартовой вершины или `не выбран`)
   - действия `Выбрать грань на карте` и `Выбрать старт на карте`
   - кнопка "Перегенерировать галсы"
   - ввод вершин через окно "Открыть координаты" с выбором CRS (`WGS84`/`СК-42`/`ГСК-2011`), выбором формата (`ДД`/`ГМ`/`ГМС`), масками полей и preview конвертации в WGS84
   - в этом окне кнопка `Ок` только закрывает окно; применение изменений выполняется кнопкой `Сохранить изменения` в правой панели
   - окно `Вершины галсов` (read-only) поддерживает выбор CRS (`WGS84`/`СК-42`/`ГСК-2011`) и формата отображения (`ДД`/`ГМ`/`ГМС`) для просмотра координат галсов

3) Маркер (`marker`):
   - имя
   - описание (многострочно)
   - ввод координаты через окно "Открыть координаты" с выбором CRS (`WGS84`/`СК-42`/`ГСК-2011`), выбором формата (`ДД`/`ГМ`/`ГМС`), масками полей и preview конвертации в WGS84
   - в этом окне кнопка `Ок` только закрывает окно; применение изменений выполняется кнопкой `Сохранить изменения` в правой панели

4) RWLT-буй (`rwlt_buoy`):
   - имя
   - цвет маркера (`marker_color`, HEX `#RRGGBB`)
   - размер маркера (`marker_size_px`, `1..256`)
   - для буя доступны общие object-действия: выбор, `Переместиться к` (`flyTo`), переименование
   - при выборе буя в списке объектов HUD переключается на телеметрию этого буя (координаты + `SOG` + глубина)
   - при последующем выборе агента или базовой станции в левой панели HUD переключается на выбранного агента/базовую станцию

5) Измерение (`measure`):
   - имя
   - описание (многострочно)
   - цвет линии
   - расстояние между двумя точками (read-only, в метрах)
...
- В каждый момент времени активен один инструмент (режим): `Выбор/Редактирование`, `Маршрут`, `Зона (галсы)`, `Маркер`, `Измерить`.
...
### 8.4.1 Измерение расстояния (`measure`)

Создание:

1) Инструмент `Измерить`.
2) Первый клик по карте фиксирует стартовую точку.
3) До второго клика отображается динамическая линия-превью от старта до курсора.
4) Второй клик фиксирует конечную точку, создаёт объект `measure`, отображает пунктирную линию и подпись длины (`м`, 2 знака после запятой) в середине линии.
5) После создания объект появляется в списке `Объекты` и выделяется.

Управление:

- `Esc` отменяет незавершенное измерение и возвращает в `Выбор/Редактирование`.
- ПКМ по кнопке `Измерить` открывает подтверждение `Удалить все измерения?`.
...
#### 8.5.4 Изменение параметров галсов (угол, ширина, ориентация, старт)

Флоу:

1) Зона выбрана.
2) Пользователь открывает "Свойства" зоны.
3) Меняет `lane_angle_deg` или `lane_width_m` в панели и/или выбирает `грань`/`старт` на карте.
   - выбор `грани` выставляет `lane_angle_deg` по bearing этой грани и сразу запускает перегенерацию галсов.
   - выбор `старта` обновляет стартовую точку и помечает галсы как `неактуальные` до ручной перегенерации.
4) Изменение `lane_angle_deg`/`lane_width_m` помечает текущие галсы как `неактуальные`.
5) Ручная перегенерация также доступна:
   - кнопкой `Перегенерировать галсы` в правой панели,
   - или пунктом `Перегенерировать галсы` в контекстном меню зоны.
6) Оба способа используют сохраненные параметры зоны; если регенерация запущена из контекстного меню, открытая правая панель синхронизирует поля с фактическими значениями объекта.
...
5) По завершении drag (mouse up) показываем неинвазивную подсказку:
   - `Галсы неактуальны.`
6) Нажатие `Перегенерировать галсы` (в правой панели или в контекстном меню зоны):
