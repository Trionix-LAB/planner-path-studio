...
- "Черновик" -> Экран 2 (Рабочий экран карты) в режиме черновика.
- "Восстановить черновик" -> Экран 2 (Рабочий экран карты) + восстановленное состояние UI/объектов.
...
- Правая панель (dock):
  - HUD (координаты, скорость, курс, глубина).
  - Статусы: связь (ОК/таймаут), запись трека (идет/пауза), активный трек (id/номер).
...
- Левая панель (dock):
  ...
  - (опционально) "Треки" (список треков в миссии с заметками и временем).
...
1) Реальное время:
   ...
   - трек рисуется согласно записи и сегментации (см. `docs/spec.md`).
   - при таймауте связи: визуальная индикация (например, значок/цвет + текст "Нет данных N сек").

2) Режим слежения:
   - toggle "Follow" (в toolbar или в HUD).
   - при включенном режиме карта центрируется на водолазе; пользовательский pan временно отключает follow до повторного включения.
...
- Разрывы по `segment_id` сохраняются как сегменты/раздельные линии (см. `docs/spec.md`).
...
- Блок "Оборудование"
 - Отображает выбранное ранее оборудование
 - Рядом с оборудование кнопка включить/выключить
 - Должен отображаться статус подключения и работы 

Правила хранения:

- Глобальные дефолты: настройки приложения.
- Переопределения для миссии: в `mission.json` (см. `docs/spec.md`, `docs/mission-format.md`).
...
- Слева список доступного для настройки оборудования.
(На этапе MVP это только Zima2R)
