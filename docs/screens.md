...
- Верхняя панель (toolbar):
  ...
  - В меню импорта должен быть явный путь для CAD-слоев:
    ...
    - для ускорения повторного открытия миссии после первого импорта/парсинга сохраняется персистентный cache-файл геометрии (`overlays/*.vector-cache.json`), который переиспользуется при загрузке миссии.
