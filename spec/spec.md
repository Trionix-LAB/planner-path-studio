...
R-058 — Растровые наложения GeoTIFF/TFW:
   ...
   - Импорт MUST быть доступен через иерархическое меню миссии верхней панели (`Черновик/Миссия` рядом с `Новая миссия`, `Экспорт`, `Скачать карты`) по пути:
     - `Импорт` -> `Импорт TIF` -> `Встроенная привязка (GeoTIFF)` — режим только GeoTIFF;
	   ...
	   - `Импорт` -> `Импорт TIF` -> `TIF + TFW (Меркатор, EPSG:3857)` — режим пары `tif/tiff + tfw` в метрах `EPSG:3857` с преобразованием bounds в `EPSG:4326`;
   ...
   - Для `TIF + TFW` режим интерпретации единиц (`градусы` / `Меркатор` / `UTM`) MUST явно выбираться пользователем при старте импорта.
