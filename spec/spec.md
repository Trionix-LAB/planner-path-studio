...
18. В одной миссии поддерживается несколько треков.

    Трек в миссии = "сессия записи" (например: до/после паузы, отдельные заплывы, разные участки работ).
19. Управление записью трека (минимум для MVP):
   - в режиме миссии запись трека включена по умолчанию
   - пользователь может поставить запись на паузу и возобновить
   - во время паузы: трек не пополняется (точки не пишутся в CSV), но метка водолаза/ HUD продолжают обновляться в реальном времени
   - пауза закрывает текущий активный трек (ставится `ended_at`), запись выключается
   - при возобновлении после паузы создается новый активный трек в миссии (новый CSV-файл)
...
- Контекст: Пользователь должен явно выбирать, трек какого агента записывать в данный момент. Это позволяет исключать лишние или шумные данные при работе с несколькими объектами.
- Интерфейс: Список доступных агентов/объектов должен содержать переключатель (toggle) или элемент управления для включения/отключения записи трека для каждого агента отдельно.
- Визуализация: При включении записи для агента UI показывает визуальный индикатор записи (иконка, цвет, подпись) как на панели объектов, так и на карте.
- Управление: Пользователь может начать, остановить или приостановить запись для выбранного агента в любой момент.
- Сохранение: Информация о том, какие агенты записываются, сохраняется в рамках сессии/миссии и восстанавливается при переоткрытии приложения.
- Acceptance: 
  ...
  - [ ] Данные о состояние записи сохраняются в структуре миссии
