...
R-058 — Растровые наложения GeoTIFF/TFW:
   ...
   - Данные растров MUST храниться отдельными файлами в папке миссии (например `overlays/*`), а не inline в `mission.json`.
   - При импорте в открытую миссию приложение MUST копировать исходные растровые файлы в папку миссии `overlays/`:
...
R-062 — Импорт DWG/DXF с UTM-параметрами:
   ...
   - Файл импортированного слоя MUST копироваться в папку миссии `overlays/` в исходном формате (`DWG` или `DXF`).
   - Для каждого векторного оверлея MUST сохраняться персистентный кэш распарсенной WGS84-геометрии и bounds в отдельном JSON-файле в папке миссии (`overlays/*.vector-cache.json`), путь к которому хранится в `mission.ui.vector_overlays[].cache_file`.
